...
import { HelpCircle, Mic, Keyboard, Command, Volume2, Eye, User, MessageSquare, Type } from "lucide-react"
    ...
    <div className="container max-w-4xl px-4 py-12 md:py-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
        {accessibilityFeatures.map((item, i) => (
          <div key={i} className="p-8 rounded-3xl border-2 bg-card space-y-4 hover:shadow-lg transition-shadow">
      ...
      <section className="mb-20 p-8 md:p-12 rounded-3xl border-2 bg-card space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <div className="space-y-6">
            <div className="space-y-2">
              <label className="text-xl font-bold">Select Font</label>
              <Select
                value={staged.fontFamily}
                onValueChange={(val: any) => setStaged((prev) => ({ ...prev, fontFamily: val }))}
              >
                <SelectTrigger className="w-full h-12 text-lg">
                  <SelectValue placeholder="Select a font" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="default">System Sans-Serif (Default)</SelectItem>
            ...
            <div className="space-y-4">
              <div className="flex justify-between">
                ...
                <span className="text-primary font-mono">{staged.fontSize}%</span>
              </div>
              <Slider
                value={[staged.fontSize]}
                onValueChange={([val]) => setStaged((prev) => ({ ...prev, fontSize: val }))}
                min={80}
                max={150}
                step={5}
              />
          ...
          <div className="space-y-6">
            <div className="space-y-4">
              <div className="flex justify-between">
                ...
                <span className="text-primary font-mono">{staged.lineHeight.toFixed(1)}x</span>
              </div>
              <Slider
                value={[staged.lineHeight]}
                onValueChange={([val]) => setStaged((prev) => ({ ...prev, lineHeight: val }))}
                min={1.2}
                max={2.5}
                step={0.1}
              />
            ...
            <div className="space-y-4">
              <div className="flex justify-between">
                ...
                <span className="text-primary font-mono">{staged.letterSpacing.toFixed(2)}em</span>
              </div>
              <Slider
                value={[staged.letterSpacing]}
                onValueChange={([val]) => setStaged((prev) => ({ ...prev, letterSpacing: val }))}
                min={0}
                max={0.3}
                step={0.01}
              />
        ...
        <div className="space-y-6">
          ...
          <div
            className={`p-8 rounded-2xl bg-secondary/30 text-center transition-all ${staged.fontFamily === "dyslexic" ? "font-dyslexic" : staged.fontFamily === "comic" ? "font-comic" : staged.fontFamily === "lexend" ? "font-lexend" : "font-sans"}`}
            style={{
          ...
          >
            The quick brown fox jumps over the lazy dog. Adjust the controls above to see how the text changes in
            real-time before applying it to the entire website.
          </div>

          <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
            <Button size="lg" className="h-14 px-8 rounded-xl font-bold gap-2" onClick={handleApply}>
            ...
            <Button
              size="lg"
              variant="outline"
              className="h-14 px-8 rounded-xl font-bold gap-2 bg-transparent"
              onClick={handleReset}
            >
      ...
      <section className="mb-20 p-8 md:p-12 rounded-3xl border-2 bg-gradient-to-br from-primary/5 to-secondary/10">
        ...
        <div className="space-y-6 text-lg text-muted-foreground leading-relaxed">
          <div className="space-y-3">
            <h3 className="text-2xl font-bold text-foreground">Activating the Assistant</h3>
            <ol className="list-decimal list-inside space-y-2 pl-4">
              <li>Say "Hey Assistant" anywhere on the site to open the AI helper</li>
              <li>Click the microphone button in the bottom-right corner to toggle wake-word listening</li>
              <li>Use Alt+A hotkey to focus the assistant instantly</li>
            </ol>
          </div>

          <div className="space-y-3">
            <h3 className="text-2xl font-bold text-foreground">Voice Conversation Mode</h3>
            <ol className="list-decimal list-inside space-y-2 pl-4">
              <li>Open the assistant and click "Start Voice Conversation"</li>
              <li>Speak your questions naturally - no need to press any buttons</li>
              <li>The assistant will respond with audio using the opposite gender voice</li>
              <li>Click "Stop Voice Conversation" when you're done</li>
            </ol>
          </div>

          <div className="space-y-3">
            <h3 className="text-2xl font-bold text-foreground">Audio Input & Output</h3>
            <ul className="list-disc list-inside space-y-2 pl-4">
              <li>Chat messages are automatically read aloud when received</li>
              <li>Male users hear female assistant voices, female users hear male voices</li>
              <li>Click the user icon in the assistant to change your gender preference</li>
            ...
            <tbody>
              {hotkeys.map((hotkey, i) => (
                <tr key={i} className="border-b last:border-0 hover:bg-secondary/20 transition-colors">
                  ...
                  <td className="px-6 py-6 text-xl text-muted-foreground">{hotkey.action}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <Accordion type="single" collapsible className="w-full">
          <AccordionItem value="item-1" className="border-b-2 py-4">
            ...
            <AccordionContent className="text-xl text-muted-foreground leading-relaxed pt-4">
              Simply click the "Upload Video" button on the home page or say "Start Submission" to the assistant. You'll
              be prompted to upload a video file of your project demo (max 100MB).
            </AccordionContent>
          </AccordionItem>
          <AccordionItem value="item-2" className="border-b-2 py-4">
            ...
            <AccordionContent className="text-xl text-muted-foreground leading-relaxed pt-4">
              We support standard video formats including MP4, MOV, WEBM, and AVI. The maximum file size is 100MB.
              Larger files will be rejected with an audio notification.
            </AccordionContent>
          </AccordionItem>
          <AccordionItem value="item-3" className="border-b-2 py-4">
            ...
            <AccordionContent className="text-xl text-muted-foreground leading-relaxed pt-4">
              The site continuously listens for "Hey Assistant" when wake-word mode is enabled (indicated by the pulsing
              microphone icon). Once detected, the assistant opens automatically and confirms with audio. You can
              disable this feature by clicking the microphone button.
            </AccordionContent>
          </AccordionItem>
          <AccordionItem value="item-4" className="border-b-2 py-4">
            ...
            <AccordionContent className="text-xl text-muted-foreground leading-relaxed pt-4">
              ...
              everything from the project title to the tech stack. After 30 seconds, you'll be prompted to share
          ...
          <AccordionItem value="item-5" className="border-b-2 py-4">
            ...
            <AccordionContent className="text-xl text-muted-foreground leading-relaxed pt-4">
              After completing your submission, you'll receive a feedback prompt after 30 seconds. You can share
              feedback via audio recording or a text form. If you start typing, audio recording automatically pauses.
              Visit /feedback anytime to share your experience.
            </AccordionContent>
          </AccordionItem>
        </Accordion>
      </section>
